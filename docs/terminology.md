...
6. **未訳のページへは既定ロケールへ落とし、リンク文字列に `(日本語)` と印を付ける。** 部分訳を出すときの標準的な作法であり、読者は「まだ訳が無い」ことをクリックする前に知れる。訳が着地したら印を外して `en/` 側へ差し替える。`test/guide.test.ts` の相対リンク検査があるので、落とし忘れ・戻し忘れはビルドで落ちる。
7. **ロケール切替は全ページの1行目に置く。** 既定ロケール側は `[English](en/…) · **日本語**`、訳側は `**English** · [日本語](../…)`。`test/i18n.test.ts` が形を検査する。
8. **図は共有する。** SVG は言語非依存なので `guide/img/` を両ロケールから参照する (訳側からは `../img/…`)。図の alt テキストだけは訳す。
