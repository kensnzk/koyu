...
|---|---|---|
| **The muro version** | **the language, its semantics and the rules of composition.** "A file written this way keeps meaning this" | **1.3** |
| **The koyu version** | **the implementation.** "This implementation's surfaces will not break." It declares which muro it implements — `koyu --version`, `package.json`'s `muro` field, and the MCP `serverInfo` all say so, from one ledger | **0.22.0** |
...
As things stand, **muro has reached 1.3 and koyu is still 0.x.**
...
|---|---|
| **The grammar and semantics of muro 1.3** | what could be read at this version stays readable with the same meaning |
...
| **The public API and the CLI** | parse, compose, canonicalise, question, derive. Commands (16 of them), arguments, exit codes |
...
- **The MCP toolset** (14 tools) — treated as a face that grows by addition
