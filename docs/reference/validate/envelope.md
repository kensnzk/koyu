...
「外皮を書き始めている」の判定は具体的である。**領域を持たない空間 (`space /out exterior` のような、範囲を書かない外部) との境界が、そのレベルのどこかに一本でも宣言されていること。**逆に言えば、外部を `space /out/n exterior X1..X3 Y2..Y3` のように**領域つきで**書いて敷地をタイルしている模型では、その境界は「領域を持つ空間どうし」なので、このレベルは外皮を書き始めたとは見なされない。
