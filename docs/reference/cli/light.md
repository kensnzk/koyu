...
The threshold lives in exactly one place. It used to be written here as well, so this command could say "needs 1/7 ≈ 2.31 m2" and print a `✔` or `✖` per room — two implementations of the same rule, which is how a tool and its own gate eventually disagree. For CI, `validate` is the gate, and it covers the other 15 rules at the same time.
