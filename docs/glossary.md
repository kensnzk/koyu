...
| 軸の語 (x+/x-/y+/y-) | Words about axes: N=+Y, S=-Y, E=+X, W=-X. The names come from drawing with north up; **which way the model actually faces is [`azimuth`](reference/muro/azimuth.md)**, and these four words do not move with it. `edge` points in this compass **as seen from the rectangle of the a side, the space written first** | [orientation](reference/muro/orientation.md) |
| frame, 枠 | Where the model's numbers sit on the earth — [`origin`](reference/muro/origin.md) (a position in a coordinate reference system, in metres) and [`azimuth`](reference/muro/azimuth.md) (the true bearing of +Y). Both are survey givens, both optional, and **koyu derives nothing from either** | [origin](reference/muro/origin.md) |
...
| type, 型 | The second positional of `space` (**optional**). **What the room is for** — `office`, `stair`, `parking`, `厨房`. An open vocabulary that **core never reads**; it appears in aggregation and in the lettering on a plan, and a judgement may take it as a population. Facts of composition (outside, void, daylight) live in the declarations `outside:` `void:` `daylight:`, and a division of the building that is not the room's purpose is a namespaced key | [space](reference/muro/space.md) |
...
| asset, アセット | A named reusable definition. Door and window assets supply defaults to openings; component assets supply fixed plan dimensions and an external SVG to a named area. It is not a fourth element and does not create geometry by itself | [asset](reference/muro/asset.md) |
...
| uncounted subdivision, 数えない分節 | A division that affects neither area, room count, graph, nor passage. `area` is a region inside a space and may also host one component; `seg` is an interval along a boundary | [area](reference/muro/area.md) / [seg](reference/muro/seg.md) |
...
| interpreted tier, 解釈層 | Attributes that are read and affect numbers or display (`w`, `h`, `t`, `sill`, `ceiling`, `name`, `at`, …). Their value form is checked, and a value written but not interpreted becomes a diagnostic | [attributes](reference/muro/attributes.md) |
...
| `edge` | The attribute restricting a boundary's segment to a particular side (x+/x-/y+/y-) of the a-side rectangle. Needed when placing an opening on an envelope whose segment splits in several | [boundary](reference/muro/boundary.md) |
...
| language version | `muro 1.5` — the version of the semantics of the notation, written once in the base layer. **Omitted, the file is read as `1.1` — and always will be.** The word is `muro` from 1.2 and `koyu` at 1.1 and earlier | [version](reference/muro/version.md) |
...
| diagnostic code | An identifier such as `BND04`. **Severity is an invariant property of the code** — the same code is never an error in one case and a warning in another. To change the weight, cut a new code. It never appears in output for people, only under `check --json` | [diagnostics](reference/diagnostics/index.md) |
...
| validation rule | A name such as `koyu.schematic.site.escape`. Its `level` is one of `violation` (it was not met) or `caution` (it is suspect). **That is architectural weight**, not a way the composition is broken | [validation](reference/validate/index.md) |
...
| MCP server | `koyu-mcp` — the entrance for agents. A dependency-free stdio server whose tools follow the standard loop `model_summary` → `layers` → `write_layer` → `check` | [MCP](reference/mcp/index.md) |
