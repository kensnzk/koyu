...
**3. Suppression is by run, not by pair.** Between two spaces one declaration suppresses the pair, edge-restricted or not — unchanged. The outside is not a pair: it is whatever the rest of the perimeter faces, so there is no unit to suppress. Each declared boundary takes the runs it realises and the default takes the remainder, which is `envelopeGaps` exactly. Writing `edge:y-` alone therefore leaves walls standing on N, E and W, where muro 1.3 left nothing.
