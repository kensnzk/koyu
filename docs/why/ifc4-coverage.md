...
| `IfcDoor` / `IfcWindow` | ● | [`door`](../reference/muro/door.md) / [`window`](../reference/muro/window.md) (`w` / `h` / `at` / `edge`), hand and swing (`hinge` / `swing`), explicit position `at:X2+450` (overruns and overlaps are checked). Door and window types are [`asset`](../reference/muro/asset.md); `style:` states leaf counts, sliding arrangements, automatic entrances, gates and window operations. The sill height `sill` is carried, not interpreted. Remaining: folding leaves, fire-rated sets (`fire` is carried and not interpreted), frames and junctions |
...
| `IfcTypeObject` / `IfcRelDefinesByType` | ◐ | [`asset`](../reference/muro/asset.md) holds opening and plan-component types. Wall types and other unmodelled types stay free words in `spec` |
