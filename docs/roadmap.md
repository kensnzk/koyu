...
| 実装 (npm パッケージ) | **0.17.0** | ライブラリ・CLI・MCP サーバー。0.x である間は**何が変わっても破壊的変更ではない** |
...
| 公開 API と CLI | TypeScript の名前と、CLI サブコマンドの契約 | ✅ 面を書き下し、実装との集合一致をテストが縛る |

## 残っている作業 — 通しで見ること

**面が立ったことと、面を見たことは違う。**テストが緑であることは、テストが縛っている範囲について言えるだけである。1.0.0 は引き返せないので、**縛っていない範囲を人が通しで見る**。0.17.0 はそのための版である。
