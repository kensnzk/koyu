...
| `There is more than one boundary segment; pick an edge with edge:y+/E/S/W` | The boundary to the exterior falls on several edges of the room's perimeter | Pick the edge on the opening with `edge:`. `N`=+Y / `S`=−Y / `E`=+X / `W`=−X | [OPN05](../reference/diagnostics/opn.md#opn05), or [SEG05](../reference/diagnostics/seg.md) for `seg` |
...
| `uses a 1.5 opening style:` / `uses a 1.5 opening attribute:` | Opening presentation introduced in 1.5 appears in a file declaring 1.4 or earlier | Raise it to `muro 1.6` | [VER08](../reference/diagnostics/ver.md#ver08) |
