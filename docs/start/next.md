...
What `check` says stops at "what is written does not contradict itself as data". A room that cannot reach the outside, a cramped stair, a ramp that is too steep, a column standing in a doorway, a building that spills off its site — a separate surface. **A judgement is not a guarantee.** That line itself is drawn in **[Scope — what a green check means](../reference/scope.md)**.
