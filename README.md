...
**[docs/](docs/index.md) is the documentation, and it is authoritative.** Laid out by what you came to do.
...
The MCP server and the CLI are the half that *judges*; the knowledge for working in muro ships separately as agent skills, loaded locally by the client so they cost nothing per call — [koyu-author](skills/koyu-author/SKILL.md) writes muro, [koyu-validate](skills/koyu-validate/SKILL.md) judges a building and explains what the findings name, [koyu-revise](skills/koyu-revise/SKILL.md) changes one without breaking the rest. One skill per question the processor answers; what building to write is not one of them. Installation (Claude Code, Claude.ai, Claude Desktop) is in [skills/README.md](skills/README.md).
