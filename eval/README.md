...
- **`light` (採光) は縮小に対して逆向きに動く。** 床が減れば必要窓面積も減るので、
  ...
  必ず面積オラクルと対にすること。
- **`light` は「型の付け替え」で母集団ごと逃げられる。** `daylight` が見るのは居室
  (`unit / room / ldk / bedroom / living`) だけである。落ちそうな室の `type` を `stor` などに変えると、
  その室は評価対象から外れ、`light` は残った室だけを見て緑になる。`score.ts` は「評価対象が0室」の
  真空の真だけは落とすが、**部分的に母集団を削る形は素通りする** (T03 の設計中に実測で見つけた穴。
  参照解の `/Ln/B{1,2}/ldk` を `stor` に変えるだけで 10 オラクル全てが緑になった)。
