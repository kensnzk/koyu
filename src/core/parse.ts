...
const EDGES = new Set(["y+", "x+", "y-", "x-", "N", "E", "S", "W"]);
...
function parseOpening(
  ...
  if (hingeRaw !== undefined && !EDGES.has(hingeRaw)) {
    throw new SourceError(ln, `hinge is given as x+/x-/y+/y- (N/E/S/W up to muro 1.5): ${hingeRaw}`);
  }
...
function applyBoundaryAttr(
  ...
  if (key === "type") {
    ...
    if (!EDGES.has(String(v))) throw new SourceError(ln, `edge is given as x+/x-/y+/y- (N/E/S/W up to muro 1.5): ${v}`);
...
function takeEdge(attrs: Attrs, ln: number): Edge | undefined {
  ...
  if (v === undefined) return undefined;
  if (!EDGES.has(v)) throw new SourceError(ln, `edge is given as x+/x-/y+/y- (N/E/S/W up to muro 1.5): ${v}`);
  return v as Edge;
}
