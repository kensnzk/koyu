...
## 桁を一つ上げる — 延床31,606㎡ を測る (2026-07-27時点の実測)

`examples/complex/` は延床31,606㎡・地下2階＋地上19階の複合建築 (商業・機械階・事務所・ホテル) である。towerの6.6倍の床面積を、同じ物差しで測った。

| | tower | complex | 倍率 |
|---|---:|---:|---:|
| 延床 | 4,786㎡ | **31,606㎡** | 6.6x |
| 空間数 | 178 | **425** | 2.4x |
| 境界数 | 543 | **1,364** | 2.5x |
| 原本の行数 | 454 | **647** | 1.43x |
| 原本のトークン (o200k) | 8,574 | **12,670** | **1.48x** |
| 正準JSON (展開後・トークン) | 74,690 | 137,899 | 1.85x |

**床面積が6.6倍になって、原本は1.48倍にしかならない。**

### 予測は外れた。外れ方に意味がある

MUN-167 の見積りは「空間数700〜1,000 → **3〜5万トークン / 1,700〜2,400行**」だった。実測は **12,670トークン / 647行**で、予測の**3〜4分の1**である。外れた理由は二つある。

**1. 空間数の見積りが過大だった (425 vs 700〜1,000)。**駐車場を区画で数えず一つの空間として持ったこと、事務所の基準階を三つの貸室として持ったことによる。これは解像度の選択であって、より細かく割ることもできる — 実際、基壇を14区画×5層のテナントに割り直したときも、増えたのは帯の宣言12行と開口の宣言だけで、**空間は84増えて原本は170行しか増えなかった** (1空間あたり2行)。
...
12,670トークンは、どのLLMのコンテキストにも余裕で収まる。参照点として、LLM×BIM編集ベンチマーク BIM-Edit (arXiv 2606.20146) の「複雑」シーンは平均615要素・2,089関係で、**しかもIFCをコンテキストに入れずプログラム経由で触らせている**。ここでは1,364境界の一棟が丸ごと文脈に載り、しかも**全体の6%にあたる一枚のレイヤーだけを差し替えれば一つの階を書き換えられる**。
...
* **斜め** — 空間が名詞、線が動詞、境界はその出会い ([ADR-0022](../../docs/decisions/0022-lines.md))。基壇の隅切りは `line X1,Y5+2000 X2,Y6` と書かれ、**読めば斜めと分かる**。頂点座標はどこにも無い。設計された斜め (斜めの貫通通路) は一度書いて**捨てた** — 規則正しい区画の並ぶモールに斜めを通すのは、記法ができるからやっただけで設計判断ではなかったからである。**線の本数は設計判断の数と一致すべきで、この例に残った斜めは所与の線 (隣地境界) に従うものだけである。**
