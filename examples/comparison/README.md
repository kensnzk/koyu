...
| | tower | complex | 倍率 |
|---|---:|---:|---:|
| 延床 | 4,786㎡ | **31,924㎡** | 6.7x |
| 空間数 | 178 | **332** | 1.9x |
| 境界数 | 542 | **1,025** | 1.9x |
| 原本の行数 | 438 | **466** | 1.06x |
| 原本のトークン (o200k) | 8,235 | **9,463** | **1.15x** |
| 正準JSON (展開後) | 66,433 | 118,445 | 1.8x |

**床面積が6.7倍になって、原本は1.15倍にしかならない。**

### 予測は外れた。外れ方に意味がある

MUN-167 の見積りは「空間数700〜1,000 → **3〜5万トークン / 1,700〜2,400行**」だった。実測は **9,463トークン / 466行**で、予測の**4〜5分の1**である。外れた理由は二つある。

**1. 空間数の見積りが過大だった (332 vs 700〜1,000)。**駐車場を区画で数えず一つの空間として持ったこと、事務所の基準階を三つの貸室として持ったことによる。これは解像度の選択であって、より細かく割ることもできる — その場合も原本の行数はほとんど増えない (2の理由による)。
...
- コア (2階段・2EVバンク・便所・PS・給湯) は **B2からL19までの21レベル分が11行**である。
