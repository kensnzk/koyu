...
| `koyu 0.3` | 任意 | 省略時は最新版 0.3 の意味論。意味を固定したいファイルは書く ([language.md §2](../spec/language.md)) |
| `name …` / `unit mm` | 任意 | |
...
| 書き方 | 意味 |
|---|---|
| `koyu 0.3` | 言語版。base層 (entry) でのみ・一度だけ。対応は `0.1` `0.2` `0.3` |
...
| `unit` `room` `ldk` `bedroom` `living` | 採光 (`light`) の対象 |
...
| 要素 | 解釈される属性 |
|---|---|
| space | `type` (一部) `level` `h` `use` `hab` `road` `uid` ・ 領域 ・ `w` (帯の要素のときのみ) |
