...
`space` の第2位置引数 (型) は必須だが、**値は何でも通る**。属性キーも同じで、何でも書けてそのまま運ばれる。この開き方は意図されたもので、意味を巨大なクラス階層ではなく語彙で与える、という選択の帰結である ([spec/vocabulary.md](../spec/vocabulary.md))。
...
`nmae:` は `name:` の誤字だが、エラーにも警告にもならず、正準JSONに `"nmae": "居間"` としてそのまま出る。台帳に無い語は「間違い」ではなく「解釈されない語」だからである。
