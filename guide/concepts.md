...
`koyu 1.0` も `unit mm` も `name` も要らない。**必要なのは、grid が二軸あること、それを使う行より前にあること、level が宣言されていること、`space` に型 (第2位置引数) が付いていることだけ**である。座標は直接書かない — 位置は常に通り芯の言葉で書く ([spec/language.md §2 通り参照とオフセット](../spec/language.md))。
