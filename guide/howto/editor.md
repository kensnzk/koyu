...
`.muro` を保存すると check が走る。エラーと警告が問題パネルに出て、診断コード (`SYN01` などの [63コード](../diagnostics.md)) がそのまま表示される。手で走らせたいときはコマンドパレットの **koyu: 整合を確かめる (check)**。
...
- [diagnostics.md](../diagnostics.md) — 診断コードから直し方を引く (全63コード)
