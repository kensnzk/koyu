...
| `docs/log/` `docs/reviews/` `docs/notes/` | Work records, design reviews and working notes. **Not published.** All three are empty as of 0.21 — the names stay reserved in `website/scripts/prepare-content.mjs`, so a file put back under one of them is unpublished from the moment it lands. | |
| The loose .md files under `docs/` | `checklists.md` (what a rippling change must touch, and which gate already holds each part) is the only **unpublished** one left; `policy.md` `writing-architecture.md` `modules.md` `horizon.md` `ifc-coverage.md` `ifcx-notes.md` `terminology.md` were deleted on 2026-08-10 and **their names are still on the deny-list**, deliberately. `glossary.md` `index.md` `roadmap.md` are published, so they are held to everything `docs/` is held to. | **A loose .md whose name is not on that deny-list is published silently** — the navigation gate counts it and passes, because it is reachable. Add the name before you add the file. |
...
| `skills/` | Agent skills, one per question the processor answers — [koyu-author](skills/koyu-author/SKILL.md) writes muro (`check`), [koyu-validate](skills/koyu-validate/SKILL.md) judges a building and explains what the findings name (`validate`), [koyu-revise](skills/koyu-revise/SKILL.md) changes one without breaking the rest (`diff`). Installed by copy or zip ([skills/README.md](skills/README.md)); deliberately outside the npm package. | Its examples sit under `check:examples`. The notation subset and the rule table are restatements — if behaviour changes, fix the skill and the page under [docs/reference/](docs/reference/index.md) in the same change. **Every muro fragment in a skill must check green**; it is the most-copied text we ship. **They teach the notation, not architecture** — see law 14. |
...
2. **Green from check does not mean the building works.** All `check` says is "what is written is not self-contradictory as data" ([docs/reference/scope.md](docs/reference/scope.md)). Architectural validity is what `koyu validate` says, separately. Because the default between touching spaces is a wall, and so is a face onto the outside ([docs/reference/muro/defaults.md](docs/reference/muro/defaults.md)), a two-storey building that declares no door at all is **completely sealed while staying green**. Never claim "it works" on the grounds that it is green.
...
14. **This repository is about the notation and the processor. It does not teach architecture.** What koyu accepts, what it derives, what it reports and why belongs here in full. Deciding what building to write does not: the programme behind a brief, zoning, circulation strategy, core placement, what a building of a given kind needs that nobody asked for, dimensional systems (structural spans, lettable depth, lift cars per shaft), which of several repairs to prefer, which cautions to live with.
    **The reason is that the two travel differently.** A notation means the same thing everywhere: `boundary` is a relation between two spaces in Tokyo and in Lisbon, and what koyu derives from it does not change with the reader. Architectural knowledge does not travel at all. A corridor width, a lift count, the workable size of an escape stair, the storey height a use wants, what rooms a building of a given kind needs before anyone asks — every one of those is a country's regulation and building culture, and this repository's own version of them is Japanese practice. [docs/howto/choose-dimensions.md](docs/howto/choose-dimensions.md) said so on its own face and told the reader to replace the whole table if they design elsewhere; [docs/reference/validate/index.md](docs/reference/validate/index.md) calls its thresholds a coarse copy of the same practice. **Shipping that as though it were part of the language exports one country's conventions under the name of a notation**, and the reader who most needs the notation is the one those numbers fit worst.
    **Two questions settle a borderline case.** Would somebody building their own koyu application need it? — then it belongs here. Would leaving it out still let koyu work, and only make an agent's buildings worse? — then it does not.
    Calibration, because the line is easy to read too wide: "a boundary with no door cannot be walked through" is the language and is written here; "reach every room from the entrance through a common corridor" is architecture and is not. `t:100` and a 900mm door are what make an example read as a building and stay; a span ledger is a dimensional system and does not. Explaining what `OPN05` means stays; choosing which of five findings to fix first does not.
    **There is a second reason, and it is the one this page can act on.** Every other law here is enforceable against `src/`. An architectural claim is enforceable against nothing — there is no declaration to consult about whether a corridor should reach every room — so it never goes red, drifts in silence, and takes the page around it with it. `test/skills-boundary.test.ts` holds the part a machine can reach, which is `skills/`. Everywhere else this law is held by a person reading.
