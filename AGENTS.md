...
| `skills/` | Agent skills — [koyu-design/](skills/koyu-design/SKILL.md) teaches an agent to *write* muro (band-first, circulation before rooms, the checker loop) with worked examples. Installed by copy or zip ([skills/README.md](skills/README.md)); deliberately outside the npm package. | Its examples sit under `check:examples`. `REFERENCE.md` is a working subset of the notation — if behaviour changes, fix it and [docs/reference/muro](docs/reference/muro/index.md) in the same change. |
