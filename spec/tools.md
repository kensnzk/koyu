...
koyu v0.12.0 現在。すべてのツールは同じ導出 (semantics.md) の別の入口である — CLIは人の手、MCPはエージェント、APIはプログラム。
...
| `axo` | `-o 出力.svg` (既定 `out/axo.svg`), `-d NE\|NW\|SE\|SW` (既定 SE), `-l L1..L5` または `-l L1,L3`, `-s 縮尺`, `--no-walls`, `--ceilings` | 軸測図SVG生成 — 床・屋根・壁・柱・縦動線を投影する (ADR-0026)。実行環境もWebGLも要らないので、平面と同じ「生成して見る」手で立体を確かめられる | 0 |
