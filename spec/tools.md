...
koyu v0.16.0 現在。すべてのツールは同じ導出 (semantics.md) の別の入口である — CLIは人の手、MCPはエージェント、APIはプログラム。
...
- **正準の境界順**: `canonicalBoundaryOrder(model)` — 宣言順は正準JSONが捨てる情報なので、**境界を順に読む導出はこの並びを使う** (ADR-0041)。`Form` の境界・開口・seg の添字 (`FormSeg.boundary` ほか) はこの並びの添字である。宣言順の配列を引くと、行を入れ替えただけで別の境界の注記を読む。
