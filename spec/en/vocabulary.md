...
| uid | ★ | An optional persistent identity token (ADR-0015). Opaque, unique across the whole model (spanning space and zone), and never derived from the path. Digits alone, or whitespace, is an error. It exists for external joins across renames (sensors, registers) — references inside the repository stay on paths. **space and zone are the only elements that can carry it, and that list is closed** (elsewhere it is ATT03). To have a machine make one, use `newUids` / the MCP tool `new_uids` (ADR-0039) |
