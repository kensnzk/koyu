...
**★の値は検査される。** 台帳に載せた属性の値が台帳の型に合わなければエラーである — 数値でなければ **ATT01**、決まった語彙 (`ceiling` 0/1・`turn` R/L・`site` 0/1・`style` hinged/sliding/auto) の外なら **ATT02** (`daylight` は DAY01、`form` は RUN05、縦動線の向きは RUN02 が既に守っている)。**書いたのに解釈されなかった値を、黙って既定へ落とさない** — `site:yes` は敷地の検査を、`h:35OO` は高さ不変量を、それぞれ丸ごと無音にする (ADR-0028)。
...
| ceiling | ★ | `0` で天井を張らない (現し天井 — ADR-0024)。既定は導出 (室の輪郭 × `h`)。天井が室の輪郭と一致しない場合 (折上げ・下がり天井・連続天井) は基本計画の解像度の外であり、作図された天井を受け取る余地は将来の値の拡張に残してある |
...
| uid | ★ | 任意の永続同一性トークン (ADR-0015)。不透明・モデル全体 (space/zone横断) で一意・パスから導出しない。数字だけの形と空白はエラー。renameを跨ぐ外部join (センサー・台帳) 用 — リポジトリ内の参照は従来どおりパス。**書けるのは space と zone だけで、この一覧は閉じている** (他の要素では ATT03)。機械に作らせるなら `newUids` / MCP の `new_uids` (ADR-0039) |
...
| h | ★ | `air:1` の境界の天端高mm (手すり・腰壁。既定1100)。軸測図が読む — 値が正の数値でなければ ATT01 (ADR-0028) |
| spec / fire / sound / … | — | 自由 (specは物の名。fire/soundはM2の区画クエリで解釈予定) |
...
### column (柱 — ADR-0023)
`column <一辺mm> <レベル範囲|レベル名> [属性...]` — **位置は書かない**。通り芯の交点のうち、そのレベルに床のある (exterior でも void でもない領域つき空間の内側にある) 所に立つ (★)。ただし半屋外で上に床の無い空間 (露天のテラス・屋上庭園) は柱の敷地にならない (ADR-0030)。
...
### level
`z` (位置引数)、`h` (基準天井高)、`slab` (床組み厚)、`pitch` (範囲宣言のみ)、`underground` (1=地下) — すべて★。`slab` は床を、`h` は天井を**既に宣言している** — 面の要素 (床・天井・屋根) はこの二つから導出され、専用の語彙を持たない (ADR-0024)。屋根は「上に空間が重なっていない範囲」に架かるので、基壇の上に塔屋が載れば基壇屋上が書かずに現れる。`underground` は宣言であって推定ではない (ADR-0022) — 地盤面は敷地の事実であり、zの負値は座標系の原点の事実にすぎない。土に接する壁は境界の型でも属性でもなく **spec 語彙** が運ぶ (`spec:RC土圧壁` — 規則2)。
